...
exports.register = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT id, status, suspended FROM machines WHERE id = $1 AND secret_key = $2`,
      [id, secret_key]
    );
    ...
    if (result.rows[0].suspended) {
      return res.status(403).json({ error: 'Machine is suspended' });
    }
    await pool.query(
...
exports.updateStock = async (req, res) => {
  try {
    ...
    await pool.query(
      `UPDATE machines SET beans_level = $1, milk_level = $2, cups_level = $3 WHERE id = $4`,
      [beans, milk, cups, id]
    );
    res.json({ ok: true });
  } catch (err) {
...
exports.getPendingOrder = async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query(
      `SELECT * FROM orders WHERE machine_id = $1 AND status = 'confirmed'
       ORDER BY created_at ASC LIMIT 1`,
      [id]
    );
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT mc.*,
       ...
       WHERE mc.machine_id = $1 GROUP BY mc.id`,
      [id]
    );
...
// ── GET /machines/nearby ── EXCLUDES SUSPENDED ────────────────
exports.getNearby = async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT m.id, m.name, m.type, m.status,
              m.beans_level, m.milk_level, m.cups_level,
              b.name AS branch_name, b.area, b.latitude, b.longitude
       FROM machines m
       LEFT JOIN branches b ON m.branch_id = b.id
       WHERE m.status = 'online'
       AND (m.suspended = false OR m.suspended IS NULL)
       LIMIT 10`
    );
...
exports.getMenu = async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query(
      `SELECT id, name, icon FROM drinks WHERE machine_id = $1 AND available = true ORDER BY id`,
      [id]
    );
